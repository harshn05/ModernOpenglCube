// g++ -s -o main -O2 main7.cpp -lglfw3 -lglew32 -lopengl32 -lgdi32 -lglu32
#include <GL/glew.h>
...
const int m = 100;                         // Number of cubes to render side by
const int n = 100;
...
int main()
{
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    for (int i = 0; i < m; ++i)
    {
      for (int j = 0; j < n; ++j)
      {
        glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(i * 1.5f - (m - 1) * 0.75f, 0.0f, j * 1.5f - (n - 1) * 0.75f));
        glUniformMatrix4fv(glGetUniformLocation(shaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(model));

        glm::vec3 cubeColor = colors.at(i * n + j);
